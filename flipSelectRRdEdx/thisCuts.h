...
const Var kTrkCostheta([](const caf::SRSliceProxy *slc)
		       {
			 int muIdx = kPTrackInd(slc);
			 if( muIdx < 0 ) return -5.;
			   return (double) slc->reco.pfp[muIdx].trk.costh;
			   
			 //                        auto const& pfp = slc->reco.pfp[muIdx];
			 //                        auto const& trk = pfp.trk
		       });

const Var kTrkCosphi([](const caf::SRSliceProxy *slc)
		     {
		       
		       int muIdx = kPTrackInd(slc);
		       if( muIdx < 0 ) return -5.;
		       double phi = slc->reco.pfp[muIdx].trk.phi;
		       //return TMath::Cos(phi); //si ma il coseno
		       return phi;
		     });
...
const SpillMultiVar spillvarOpFlashTimeData([](const caf::SRSpillProxy *sr)
					{
					  std::vector<double> rets;
					  for(const auto& opflash : sr->opflashes){
					    double this_oft = opflash.firsttime;
...
const SpillMultiVar spillvarOpFlashTimeCorrectMC([](const caf::SRSpillProxy *sr)
						 {
						   std::vector<double> rets;
						   for(const auto& opflash : sr->opflashes){
...
//Multivar->Get full vector

const MultiVar kResiRange([](const caf::SRSliceProxy* slc) -> std::vector<double>{
    std::vector<double> rera;
    if ( kPTrackInd(slc) >= 0 ){
      auto const& trk = slc->reco.pfp.at(kPTrackInd(slc)).trk;
      //        for ( unsigned int ipoint = 0; ipoint < trk.calo[2].points.size(); ++ipoint ) {
      for ( auto const& point : trk.calo[2].points ) {
	rera.push_back(point.rr);
	//        }
      }
    }
    return rera;
...
const MultiVar kdEdx([](const caf::SRSliceProxy* slc) -> std::vector<double>{
    std::vector<double> dedxv;
    if ( kPTrackInd(slc) >= 0 ){
      auto const& trk = slc->reco.pfp.at(kPTrackInd(slc)).trk;
      //        for ( unsigned int idxTrk = 0; idxTrk < slc->reco.pfp.size(); ++idxTrk ) {
      for ( auto const& point : trk.calo[2].points ) {
	dedxv.push_back(point.dedx);
      }
    }
    return dedxv;
  });
...
const SpillCut OpFlash_cutTime([](const caf::SRSpillProxy* sr){
    for (const auto& opflash : sr->opflashes){
      auto thistime = opflash.time; //.time(?)// opflash.firsttime +  9999.f;
      if (thistime > -0.1 && thistime < 9.7) {
        return true;
      }
    }
...
const SpillCut OpFlash_cutLarge([](const caf::SRSpillProxy* sr){
    for (const auto& opflash : sr->opflashes){
      auto thistime =  opflash.firsttime;
      if (thistime > -100000 && thistime < 10000) {
        return true;
      }
    }
    return false;
...
const Binning dedxBinning       = Binning::Simple(30, 0., 30.);//NEW
const Binning kdedxBinningRestricted = Binning::Simple(24, 3., 15.);
const Binning kResRangeBinning = Binning::Simple(50, 0., 35.);
...
const Binning kCosThetaBinning = Binning::Simple(50,-1.,1.);
const Binning kCosPhiBinning = Binning::Simple(62,-3.1,3.1); //kCosThetaBinning,kCosPhiBinning
...
const HistAxis axdedx  ("axdedx",      dedxBinning,       dedxVar);
const HistAxis axdedxRestricted  ("axdedxSmaller", kdedxBinningRestricted,       dedxVar);
...
//const HistAxis axResRange ("ResRange",kResRangeBinning,);//UNFINISHED
