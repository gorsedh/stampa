...
const std::string fdata_majority_old = "/pnfs/icarus/scratch/users/gputnam/DMCP2023G/majority-3t1p/*1/data*Prescaled*.root";
//const std::string fdirt_mc_old = "/pnfs/sbn/data/sbn_fd/poms_production/2023A_ICARUS_NuMI_MC_dirt_plus_cosmics/pretuned_signal_shape/mc/reconstructed/icaruscode_v09_72_00_03/flatcaf/[0,3,]*/[0,6]*/detsim*.flat.caf*.root";
  ...
  Spectrum sCosTheta_allMCNu   (loader_allMC, axCosTheta, OpFlash_MC, kDiryNuTkl && !kRFiducial && kPTrackContained);
  Spectrum sCosTheta_allMCNoNu (loader_allMC, axCosTheta, OpFlash_MC, kDiryNoNuTkl && !kRFiducial && kPTrackContained);
  Spectrum sCosTheta_rockMCNu   (loader_rockMC, axCosTheta, OpFlash_MC, kDiryNuTkl && !kRFiducial && kPTrackContained);
  Spectrum sCosTheta_rockMCNoNu (loader_rockMC, axCosTheta, OpFlash_MC, kDiryNoNuTkl && !kRFiducial && kPTrackContained);
  ...
  Spectrum sCosPhi_allMCNu   (loader_allMC, axCosPhi, OpFlash_MC, kDiryNuTkl && !kRFiducial && kPTrackContained);
  Spectrum sCosPhi_allMCNoNu (loader_allMC, axCosPhi, OpFlash_MC, kDiryNoNuTkl && !kRFiducial && kPTrackContained);
  Spectrum sCosPhi_rockMCNu   (loader_rockMC, axCosPhi, OpFlash_MC, kDiryNuTkl && !kRFiducial && kPTrackContained);
  Spectrum sCosPhi_rockMCNoNu (loader_rockMC, axCosPhi, OpFlash_MC, kDiryNoNuTkl && !kRFiducial && kPTrackContained);
 ...
 //Spectrum sRRdEdx_data ("dEdxResidualRange", loader_data, kResRangeBinning, kResiRange, dedxBinning, kdEdx, kNoSpillCut, kSlcTrkDi
  ...
  sCosTheta_allMCNu.OverridePOT(1);
  sCosTheta_allMCNoNu.OverridePOT(1);
  sCosPhi_allMCNu.OverridePOT(1);
  sCosPhi_allMCNoNu.OverridePOT(1);
  //
  sCosTheta_rockMCNu.OverridePOT(1);
  sCosTheta_rockMCNoNu.OverridePOT(1);
  sCosPhi_rockMCNu.OverridePOT(1);
  sCosPhi_rockMCNoNu.OverridePOT(1);
  ...
  TH1* hCosTheta_allMCNu = sCosTheta_allMCNu.ToTH1(POT);
  TH1* hCosTheta_allMCNoNu = sCosTheta_allMCNoNu.ToTH1(POT);
  TH1* hCosPhi_allMCNu = sCosPhi_allMCNu.ToTH1(POT);
  TH1* hCosPhi_allMCNoNu = sCosPhi_allMCNoNu.ToTH1(POT);

  TH1* hCosTheta_rockMCNu = sCosTheta_rockMCNu.ToTH1(POT);
  TH1* hCosTheta_rockMCNoNu = sCosTheta_rockMCNoNu.ToTH1(POT);
  TH1* hCosPhi_rockMCNu = sCosPhi_rockMCNu.ToTH1(POT);
  TH1* hCosPhi_rockMCNoNu = sCosPhi_rockMCNoNu.ToTH1(POT);
  ...
  hCosTheta_allMCNu->SetTitle("AllMC(TrkDiryNu+!Fid/Cont+OP1stFlash);Cos#theta;Events");
  hCosTheta_allMCNoNu->SetTitle("AllMC(TrkDiryNoNu+!Fid/Cont+OP1stFlash);Cos#theta;Events");
  hCosPhi_allMCNu->SetTitle("AllMC(TrkDiryNu+!Fid/Cont+OP1stFlash);Cos#theta;Events");
  hCosPhi_allMCNoNu->SetTitle("AllMC(TrkDiryNoNu+!Fid/Cont+OP1stFlash);Cos#theta;Events");

  hCosTheta_rockMCNu->SetTitle("RockMC(TrkDiryNu+!Fid/Cont+OP1stFlash);Cos#theta;Events");
  hCosTheta_rockMCNoNu->SetTitle("RockMC(TrkDiryNoNu+!Fid/Cont+OP1stFlash);Cos#theta;Events");
  hCosPhi_rockMCNu->SetTitle("RockMC(TrkDiryNu+!Fid/Cont+OP1stFlash);Cos#theta;Events");
  hCosPhi_rockMCNoNu->SetTitle("RockMC(TrkDiryNoNu+!Fid/Cont+OP1stFlash);Cos#theta;Events");
  ...
  TCanvas *cosThetaCanvasAllMC = new TCanvas("cosThetaCanvasAllMC","cosThetaCanvasAllMC");
  hCosTheta_allMCNu->SetLineColor(kRed);
  hCosTheta_allMCNoNu->SetLineColor(kBlue);
  hCosTheta_allMCNoNu->SetFillColor(kBlue);
  hCosTheta_allMC->Draw("histo");
  hCosTheta_allMCNoNu->Draw("hist same");
  hCosTheta_allMCNu->Draw("hist same");
  TLegend *legCosThetaAllMC = new TLegend(0.15,0.7,0.35,0.9);
  legCosThetaAllMC->AddEntry(hCosTheta_allMC, "AllMC", "l");
  legCosThetaAllMC->AddEntry(hCosTheta_allMCNu, "Nu", "l");
  legCosThetaAllMC->AddEntry(hCosTheta_allMCNoNu, "NoNu", "l");
  legCosThetaAllMC->Draw();
  cosThetaCanvasAllMC->Print("Outcosthphi/cosThetaCanvasAllMC.pdf");

  TCanvas *cosThetaCanvasRockMC = new TCanvas("cosThetaCanvasRockMC","cosThetaCanvasRockMC");
  hCosTheta_rockMCNu->SetLineColor(kRed);
  hCosTheta_rockMCNoNu->SetLineColor(kBlue);
  hCosTheta_rockMCNoNu->SetFillColor(kBlue);
  hCosTheta_rockMC->SetLineColor(kBlack);
  hCosTheta_rockMC->Draw("histo");
  hCosTheta_rockMCNoNu->Draw("hist same");
  hCosTheta_rockMCNu->Draw("hist same");
  TLegend *legCosThetaRockMC = new TLegend(0.15,0.7,0.35,0.9);
  legCosThetaRockMC->AddEntry(hCosTheta_rockMC, "RockMC", "l");
  legCosThetaRockMC->AddEntry(hCosTheta_rockMCNu, "Nu", "l");
  legCosThetaRockMC->AddEntry(hCosTheta_rockMCNoNu, "NoNu", "l");
  legCosThetaRockMC->Draw();
  cosThetaCanvasRockMC->Print("Outcosthphi/cosThetaCanvasRockMC.pdf");

  TCanvas *cosPhiCanvasAllMC = new TCanvas("cosPhiCanvasAllMC","cosPhiCanvasAllMC");
  hCosPhi_allMCNu->SetLineColor(kRed);
  hCosPhi_allMCNoNu->SetLineColor(kBlue);
  hCosPhi_allMCNoNu->SetFillColor(kBlue);
  hCosPhi_allMC->Draw("histo");
  hCosPhi_allMCNoNu->Draw("hist same");
  hCosPhi_allMCNu->Draw("hist same");
  TLegend *legCosPhiAllMC = new TLegend(0.15,0.7,0.35,0.9);
  legCosPhiAllMC->AddEntry(hCosPhi_allMC, "AllMC", "l");
  legCosPhiAllMC->AddEntry(hCosPhi_allMCNu, "Nu", "l");
  legCosPhiAllMC->AddEntry(hCosPhi_allMCNoNu, "NoNu", "l");
  legCosPhiAllMC->Draw();
  cosPhiCanvasAllMC->Print("Outcosthphi/PhiCanvasAllMC.pdf");

  TCanvas *cosPhiCanvasRockMC = new TCanvas("cosPhiCanvasRockMC","cosPhiCanvasRockMC");
  hCosPhi_rockMCNu->SetLineColor(kRed);
  hCosPhi_rockMCNoNu->SetLineColor(kBlue);
  hCosPhi_rockMCNoNu->SetFillColor(kBlue);
  hCosPhi_rockMC->SetLineColor(kBlack);
  hCosPhi_rockMC->Draw("histo");
  hCosPhi_rockMCNoNu->Draw("hist same");
  hCosPhi_rockMCNu->Draw("hist same");
  TLegend *legCosPhiRockMC = new TLegend(0.15,0.7,0.35,0.9);
  legCosPhiRockMC->AddEntry(hCosPhi_rockMC, "RockMC", "l");
  legCosPhiRockMC->AddEntry(hCosPhi_rockMCNu, "Nu", "l");
  legCosPhiRockMC->AddEntry(hCosPhi_rockMCNoNu, "NoNu", "l");
  legCosPhiRockMC->Draw();
  cosPhiCanvasRockMC->Print("Outcosthphi/PhiCanvasRockMC.pdf");

  //TCanvas *cosPhiCanvasAllMCRockMC = new TCanvas("cosPhiCanvasAllMCRockMC","cosPhiCanvasAllMCRockMC");
  //cosPhiCanvasAllMCRockMC->Divide(2,2);
  // cosPhiCanvasAllMCRockMC->cd(1);
